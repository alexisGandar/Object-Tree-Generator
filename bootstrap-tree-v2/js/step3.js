		...
		*/
		chooseExport : function(){
			...
			$('.popup .close').click(function (e) {
			 ...
			 $('#cancel').click(app.modules.step3.hideChooseFill);
			 $('#send').click(function(){
				 $(id).html('<label>Please spéecify the URL of the send request</label><input class="form-control" id="inputCustom"><button class="btn custom btn-default" id="confirm">Confirm</button><button class="btn custom btn-default" id="cancel">cancel</button>');
			 ...
			 xml_tree[0].nodes.forEach(function(e){
				 ...
				 listXml_tree.forEach(function(y){
					 y.nodes.forEach(function(k){
						 if(k.text == e.text){
							 s = app.modules.step3.buildXml(type,y.nodes,2,s);
						 }
					 });
				 });
				 s = s + "\t</Object>\n";
